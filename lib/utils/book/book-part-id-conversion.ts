...
/** string | number  →  number | null  (for form field / DB column) */
export function toFormBookPart(
    bookPart: string | number | undefined
): number | null {
    if (bookPart === "" || bookPart === undefined || bookPart === null)
        return null;
    const parsed = Number(bookPart);
    return isNaN(parsed) ? null : parsed;
}
